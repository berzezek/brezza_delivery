...
async def cmd_start(message: Message, state: FSMContext):
    ...
    await state.update_data(receiver_name=message.from_user.full_name)
...
async def customer_enter(message: Message, state: FSMContext):
    ...
    if response:
        ...
        await send_success_subscribe_message(message, state, customer['title'], customer['description'])
...
async def accept_order(callback: CallbackQuery, state: FSMContext):
    ...
    if order:
        ...
        send_message_to_tg_user(
            f"Доставка для {customer} обработана в {timenow}")
